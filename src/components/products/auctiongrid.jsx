...
export default function auctiongrid() {
  ...
  const getdata = (page) => {
    const perPage = 8;
    // http://localhost:8080/api/buy/allBuy?page=${page}&perPage=${perPage}
    fetch(
      `${baseurl}/api/auction/allAuction?page=${page}&perPage=${perPage}`
    )
      .then((res) => {
        return res.json();
      })
  ...
  const handlecart = (product) => {
    ...
    fetch(`${baseurl}/api/rentcart/addCartRent`, {
      method: "POST",
      headers: {
                  ...
                  <Grid item xs={6} md={4} sm={6} lg={3} xl={2} key={index}>
                    <div id="productcard">
                      ...
                      <div
                        id="productcardtitle"
                        onClick={() =>
                          router.push(`/products/rent/${product.id}`)
                        }
                      >
